"""
SKYNET Agent Executor
Runs multi-step plans, handles retries, replanning, and result injection.
"""
...
def _detect_language(text: str) -> str:
    ...
    try:
        response = model.generate_content(
            f"What language is this text? Reply with ONLY the language name in English "
            f"(e.g. Persian, English, Turkish).\n\nText: {text[:200]}"
        )
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    """Translate content to match the language of the goal."""
    if not goal:
        return content
    try:
        import google.generativeai as genai
        ...
        prompt = (
            ...
            f"Rules: translate everything, keep all facts/numbers, keep structure.\n"
        ...
        print(f"[Executor] ⚠️  Translation failed: {exc}")
        return content


def _inject_context(params: dict, tool: str, step_results: dict, goal: str = "") -> dict:
    """Inject prior step results into a write/create step if content is missing."""
    if not step_results:
        return params
    params = dict(params)
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content or len(content) < 50:
            ...
            if useful:
                ...
                print("[Executor] 💉 Injected + translated content into write step")
    return params
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        return flight_finder(parameters=parameters, player=None) or "Done."
...
class AgentExecutor:
    ...
    def execute(
        ...
        while True:
            ...
            if not steps:
                msg = "Could not create a valid plan for this task."
                if speak:
                    speak(msg)
                return msg
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            if fix_suggestion:
                                try:
                                    ...
                                    if speak:
                                        speak("Trying alternative approach.")
                                    res = _call_tool(
                                        fixed_step["tool"],
            ...
            if speak:
                speak("Adjusting approach.")
            replan_attempts += 1
            plan = replan(goal, completed_steps, failed_step, failed_error)

    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        fallback = f"Completed {len(completed_steps)} steps for: {goal[:60]}."
        try:
            import google.generativeai as genai
            genai.configure(api_key=_get_api_key())
            model     = genai.GenerativeModel("gemini-2.5-flash-lite")
            ...
            # Detect language of the goal to reply in the same language
            lang  = _detect_language(goal)
            prompt = (
                f'User goal: "{goal}"\n'
                f"Completed steps:\n{steps_str}\n\n"
                f"Write a single natural sentence summarizing what was accomplished. "
                f"Reply in {lang}. Be direct and positive."
            )
            response = model.generate_content(prompt)
